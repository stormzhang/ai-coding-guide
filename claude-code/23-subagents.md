...
顺带澄清一个说法变过几回的事：**子代理能不能再派生子代理（嵌套）**。嵌套派生 v2.1.172 就支持了，v2.1.217 一度改成默认禁止，v2.1.219 又恢复成默认允许。当前官方文档的口径是：**嵌套深度上限 5 层、固定不可调**——深度到 5 的子代理拿不到 `Agent` 工具，自然没法再往下派；想单独禁掉某个子代理的派生能力，从它的 `tools` 里去掉 `Agent` 就行。
